...
import { Trophy, CheckCircle, ExternalLink, MessageSquare } from "lucide-react";
...
const facebookTasks = Array.from({ length: 12 }, (_, i) => ({
    ...
    title: singleCampaign.postTitle,
}));
...
export default function FacebookLikePage() {
    ...
    const handleCompleteTask = (taskId: number) => {
        ...
        setCompletedTasks(prev => new Set(prev).add(taskId));
         ...
         // In a real app, you would open the link to the buyer's Facebook post:
         // window.open('https://facebook.com/SOME_POST_ID', '_blank');
    }
                    ...
                    return (
                        <Card key={task.id} className={isCompleted ? 'bg-muted/50' : ''}>
                            <CardHeader>
                                ...
                                <CardDescription>Post: <strong>{task.title}</strong></CardDescription>
                            </CardHeader>
                            <CardContent>
